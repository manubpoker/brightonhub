...
const navItems: NavItem[] = [
  ...
  { href: '/transport', label: 'Transport', icon: Train, underConstruction: true },
  { href: '/planning', label: 'Planning', icon: Landmark, underConstruction: true },
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      ...
      {/* Mobile nav */}
      {mobileMenuOpen && (
        <nav className="lg:hidden border-t bg-white px-4 pb-4 pt-2">
          {navItems.map(({ href, label, icon: Icon, underConstruction }) => (
            <Link
              key={href}
              href={href}
